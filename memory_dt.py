        ...
        if self.memory is not None:
            if self.memory_type == 'gru':
                if self.hidden_state is None:
                    # TODO: Implement GRU memory
                    self.hidden_state = torch.zeros(1, batch_size, self.memory.hidden_size, device=state_embeddings.device)
                memory_out, self.hidden_state = self.memory(state_embeddings, self.hidden_state)
            elif self.memory_type == 'lstm':
                if self.hidden_state is None:
                    # TODO: Implement LSTM memory
                    h0 = torch.zeros(
                        1, batch_size, self.memory.hidden_size, device=state_embeddings.device
                    )
                    c0 = torch.zeros(
                        1, batch_size, self.memory.hidden_size, device=state_embeddings.device
                    )
                ...
                memory_out, self.hidden_state = self.memory(state_embeddings, self.hidden_state)
            elif self.memory_type == 'tcn':
                # [B, T, E] -> [B, E, T]
                x = state_embeddings.transpose(1, 2)

                memory_out = self.memory(x)

                # remove extra padding effects
                memory_out = memory_out[:, :, :seq_length]

                # [B, E, T] -> [B, T, E]
                memory_out = memory_out.transpose(1, 2) 
